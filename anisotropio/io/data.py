...
import anisotropio.utils.trace as trace_utils
...
class Archive:
    ...
    :class:`~anisotropio.io.data.WaveformData` object.
    ...
    decimated to 50 Hz - see :func:`~anisotropio.utils.trace.resample`.
    ...
    stations : `pandas.DataFrame` object
        ...
        :func:`~anisotropio.io.core.read_stations`
    archive_format : str, optional
        Sets directory structure and file naming format for different archive
        formats. See :func:`~anisotropio.io.data.Archive.path_structure`
    kwargs : **dict
    ...
    resample : bool, optional
        ...
        See :func:`~anisotropio.utils.trace.resample`
    ...
    interpolate : bool, optional
        ...
        :func:`~anisotropio.utils.trace.shift_to_sample`.
    ...
    def __str__(self, response_only=False):
        ...
        if not response_only:
            out = ("AnisotropIO Archive object"
    ...
    def path_structure(self, archive_format="YEAR/JD/STATION", channels="*"):
        ...
        archive_format : str, optional
            ...
            or one of a selection of additional formats built into AnisotropIO.
        ...
        if archive_format == "SeisComp3":
            ...
            raise errors.InvalidArchivePathStructure(archive_format)
    ...
    def read_waveform_data(self, starttime, endtime, pre_pad=0., post_pad=0.):
        ...
        Returns
        -------
        data : :class:`~anisotropio.io.data.WaveformData` object
        ...
        try:
            ...
            # migrated.
            st = trace_utils.shift_to_sample(st, interpolate=self.interpolate)
            ...
            if not bool(st):
                raise errors.AllTracesHaveGaps

            # Add cleaned stream to `waveforms`
            data.waveforms = st

        except StopIteration:
            raise errors.ArchiveEmpty
    ...
    def _load_from_path(self, starttime, endtime):
        ...
        if self.format is None:
            raise errors.UnsetArchiveFormat
...
class WaveformData:
        ...
        if bool(st):
            ...
            for tr_id in sorted(set([tr.id for tr in st])):
                ...
                if check_sampling_rate:
                    if not sampling_rate:
                        raise TypeError(
                            "Please specify sampling_rate if you wish to check"
                            " all channels are at the correct sampling rate."
                        )
            ...
            if all(ava == 1 for ava in availability.values()):
                if all_channels:
                    ...
                    if not n_channels:
                        raise TypeError(
                            "Please specify n_channels if you wish to check "
                            "all channels meet the availability criteria."
                        )
    ...
    def get_real_waveform(self, tr, velocity=True):
        ...
        if not self.remove_full_response:
            ...
            try:
                ...
                raise errors.ResponseNotFound(str(e), tr.id)
            ...
            try:
                ...
                raise errors.ResponseRemovalFailed(e, tr.id)
        else:
    ...
    def get_wa_waveform(self, tr, velocity=False):
        ...
        tr.simulate(
            paz_simulate=trace_utils.wa_response(obspy_def=True),
            pre_filt=self.pre_filt,
            water_level=self.water_level,
            taper=True,
            sacsim=True,  # To replicate remove_response()
